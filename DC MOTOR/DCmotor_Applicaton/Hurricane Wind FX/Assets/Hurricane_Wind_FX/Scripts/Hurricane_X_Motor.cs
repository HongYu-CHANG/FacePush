...
public class Hurricane_X_Motor : MonoBehaviour {
	...
	void Update () 
	{
		...
		if(timer_i % ConvertTime == 0 && !HurricaneWork)
		{
	        ...
	        var Debrismain = Debris.main;
	 		...
	 		// DC motor
	 		Debug.Log("Motor = " + (int)remapping((float)speed, 1f, 10f, 85f, 255f));
	 		StartCoroutine(SHAKEWORK());
		}
	...
	IEnumerator SHAKEWORK()
	{
		int tempSpeed = (int)remapping((float)speed, 1f, 10f, 85f, 255f);
		for(int i = 0 ; i < ConvertTime; i++)
		{
			R_OSCSender.SendOSCMessageTriggerMethod("FORWARD", tempSpeed, 1);//加壓
	        L_OSCSender.SendOSCMessageTriggerMethod("FORWARD", tempSpeed, 1);
	        yield return new WaitForSeconds(0.5f);
	        R_OSCSender.SendOSCMessageTriggerMethod("BACKWARD", tempSpeed, 1);//加壓
	        L_OSCSender.SendOSCMessageTriggerMethod("BACKWARD", tempSpeed, 1);
	        yield return new WaitForSeconds(0.5f);
    	}
    	R_OSCSender.SendOSCMessageTriggerMethod("RELEASE", 255, 2);
        L_OSCSender.SendOSCMessageTriggerMethod("RELEASE", 255, 2);
	}
}
