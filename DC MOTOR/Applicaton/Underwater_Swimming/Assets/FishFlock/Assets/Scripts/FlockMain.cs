...
namespace FishFlock
{
    ...
    public class FlockMain
    {
        ...
        static volatile List<FlockCalculationRequest> requests = new List<FlockCalculationRequest>();
        ...
        static void CalculateBehaviours()
        {
            ...
            List<FishBehaviour> behaviours = requests[0].behaviours;
            Predator[] predators = requests[0].predators;
            float separationScale = requests[0].separationScale;
            float alignmentScale = requests[0].alignmentScale;
            float cohesionScale = requests[0].cohesionScale;
            bool forceGroupAreaDestination = requests[0].forceGroupAreaDestination;
            ...
            requests[0].callback(behaviours);
            requests.RemoveAt(0);          
        }
